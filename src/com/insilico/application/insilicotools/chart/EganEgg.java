...
public class EganEgg extends JPanel {
    ...
    private final XYShapeAnnotation abs95Annotation;
    private final XYShapeAnnotation abs99Annotation;
    private final XYShapeAnnotation bbb90Annotation;
    private final XYShapeAnnotation bbb99Annotation;

    private JCheckBox abs95CheckBox;
    private JCheckBox abs99CheckBox;
    private JCheckBox bbb90CheckBox;
    private JCheckBox bbb99CheckBox;
    ...
    public static final int ABS95_ELLIPSE = 0;
    public static final int ABS99_ELLIPSE = 1;
    public static final int BBB90_ELLIPSE = 2;
    public static final int BBB99_ELLIPSE = 3;
    ...
    public EganEgg(String title, String xAxis, String yAxis) {
        this(title, xAxis, yAxis, true, true, true);
    }

    public EganEgg(String title, String xAxis, String yAxis, boolean createLegend, boolean createChecks) {
        this(title, xAxis, yAxis, createLegend, createChecks, true);
    }

    public EganEgg(String title, String xAxis, String yAxis, boolean createLegend, boolean createChecks, boolean showToolTip) {
        this(title, xAxis, yAxis, createLegend, createChecks, showToolTip, false);
    }

    public EganEgg(String title, String xAxis, String yAxis, boolean createLegend, boolean createChecks, boolean showToolTip, boolean createToolbar) {
        ...
        scatterplot = new Scatterplot(null, xAxis, yAxis, createLegend) {
            public void chartMouseClicked(ChartMouseEvent event) {
                ...
                if (isPointOnEdge(ABS95Pct, pt, dataArea)) {
                    selectPointsInShape(abs95Shape);
                } else if (isPointOnEdge(ABS99Pct, pt, dataArea)) {
                    selectPointsInShape(abs99Shape);
                } else if (isPointOnEdge(BBB90Pct, pt, dataArea)) {
                    selectPointsInShape(bbb90Shape);
                } else if (isPointOnEdge(BBB99Pct, pt, dataArea)) {
                    selectPointsInShape(bbb99Shape);
                }
            }
        };
...
//            }
                ...
                public String generateToolTip(XYDataset dataset, int series, int item) {
                    try {
                        ...
                        if(key!=null&&key instanceof PropertyMolecule) {
                            PropertyMolecule mol = (PropertyMolecule) key;
                            String smiles = String.format("%s %s",mol.getSmiles(),mol.getName());
                            return ImageUtil.smi2ImgUrl(smiles);
                        }else{
                            return null;
                        }
                    }
        ...
        final BasicStroke dashed = new BasicStroke(1.0f, BasicStroke.JOIN_BEVEL, BasicStroke.JOIN_MITER, 10.0f, new float[]{10.0f}, 0.0f);
        final BasicStroke smallDashed = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, new float[]{3.0f}, 0.0f);
        final BasicStroke solid = new BasicStroke(1.0f);

        abs95Shape = createPolygon(ABS95Pct);
        abs95Annotation = new XYShapeAnnotation(abs95Shape, solid, Color.black);
        plot.getRenderer().addAnnotation(abs95Annotation,Layer.BACKGROUND);
        Rectangle2D bounds = abs95Shape.getBounds2D();
        ...
        abs99Annotation = new XYShapeAnnotation(abs99Shape, dashed, Color.black);
        plot.getRenderer().addAnnotation(abs99Annotation,Layer.BACKGROUND);
        bounds = abs99Shape.getBounds2D();
        minAnnotationX = Math.min(minAnnotationX, bounds.getMinX());
        minAnnotationY = Math.min(minAnnotationY, bounds.getMinY());
        maxAnnotationX = Math.max(maxAnnotationX, bounds.getMaxX());
        maxAnnotationY = Math.max(maxAnnotationY, bounds.getMaxY());
        ...
        bbb90Annotation = new XYShapeAnnotation(bbb90Shape, solid, Color.BLUE);
//        plot.getRenderer().addAnnotation(bbb90Annotation,Layer.BACKGROUND);
        bounds = bbb90Shape.getBounds2D();
        minAnnotationX = Math.min(minAnnotationX, bounds.getMinX());
        minAnnotationY = Math.min(minAnnotationY, bounds.getMinY());
        maxAnnotationX = Math.max(maxAnnotationX, bounds.getMaxX());
        maxAnnotationY = Math.max(maxAnnotationY, bounds.getMaxY());
        ...
        bbb99Annotation = new XYShapeAnnotation(bbb99Shape, dashed, Color.BLUE);
//        plot.getRenderer().addAnnotation(bbb99Annotation, Layer.BACKGROUND);
        bounds = bbb99Shape.getBounds2D();
        minAnnotationX = Math.min(minAnnotationX, bounds.getMinX());
        minAnnotationY = Math.min(minAnnotationY, bounds.getMinY());
        maxAnnotationX = Math.max(maxAnnotationX, bounds.getMaxX());
        maxAnnotationY = Math.max(maxAnnotationY, bounds.getMaxY());
        ...
        if (createChecks) {
            ...
            checkBoxPanel.setBackground((Color) chart.getBackgroundPaint());
            ...
            abs95CheckBox = new JCheckBox("", true);
            abs99CheckBox = new JCheckBox("", true);
            bbb90CheckBox = new JCheckBox("", false);
            bbb99CheckBox = new JCheckBox("", false);

            checkBoxPanel.add(createCheckBoxPanel(abs95CheckBox, abs95Annotation, "Absorption 95%", solid, Color.black));
            checkBoxPanel.add(createCheckBoxPanel(abs99CheckBox, abs99Annotation, "Absorption 99%", smallDashed, Color.black));
            checkBoxPanel.add(createCheckBoxPanel(bbb90CheckBox, bbb90Annotation, "CNS 90%", solid, Color.BLUE));
            checkBoxPanel.add(createCheckBoxPanel(bbb99CheckBox, bbb99Annotation, "CNS 99%", smallDashed, Color.BLUE));
            ...
            checkBoxPanel.add(createNonCheckBoxLabel(abs95Annotation, "Absorption 95%", solid, Color.black));
            checkBoxPanel.add(createNonCheckBoxLabel(abs99Annotation, "Absorption 99%", smallDashed, Color.black));
            checkBoxPanel.add(createNonCheckBoxLabel(bbb90Annotation, "CNS 90%", solid, Color.BLUE));
            checkBoxPanel.add(createNonCheckBoxLabel(bbb99Annotation, "CNS 99%", smallDashed, Color.BLUE));
        ...
        if (createToolbar) {
            ScatterplotToolbar toolbar = new ScatterplotToolbar(JToolBar.VERTICAL, scatterplot);
    ...
    public void selectPointsInEllipse(int ellipse, boolean selected) {
        Shape shape = null;
        ...
        for (int i = 0; i < data.getSeriesCount(); i++) {
            for (int j = 0; j < data.getItemCount(i); j++) {
                if (shape.contains(data.getX(i, j).doubleValue(), data.getY(i, j).doubleValue())) {
                    data.getItem(i, j).setSelected(selected);
                }
            }
        }
    }
    ...
    private JPanel createCheckBoxPanel(JCheckBox checkBox, final XYShapeAnnotation annotation, String label, Stroke stroke, Color color) {
        checkBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (((JCheckBox) e.getSource()).isSelected()) {
                    plot.getRenderer().addAnnotation(annotation, Layer.BACKGROUND);
                } else {
                    plot.getRenderer().removeAnnotation(annotation);
                }
            }
        });
        ...
        p.add(checkBox);
        p.add(new JLabel(label, new AnnotationIcon(stroke, color), JLabel.CENTER));
        return p;
    }

    public void removeBBB() {
        plot.getRenderer().removeAnnotation(bbb90Annotation);
        plot.getRenderer().removeAnnotation(bbb99Annotation);
    }

    public void hideBBB() {
        if (bbb90CheckBox != null && bbb90CheckBox.isSelected()) {
            bbb90CheckBox.setSelected(false);
            plot.getRenderer().removeAnnotation(bbb90Annotation);
        }
        if (bbb99CheckBox != null && bbb99CheckBox.isSelected()) {
            bbb99CheckBox.setSelected(false);
            plot.getRenderer().removeAnnotation(bbb99Annotation);
        }
    }
    ...
    public void showAll() {
        if (bbb90CheckBox != null && !bbb90CheckBox.isSelected()) {
            bbb90CheckBox.setSelected(true);
            plot.getRenderer().addAnnotation(bbb90Annotation, Layer.BACKGROUND);
        }
        if (bbb99CheckBox != null && !bbb99CheckBox.isSelected()) {
            bbb99CheckBox.setSelected(true);
            plot.getRenderer().addAnnotation(bbb99Annotation, Layer.BACKGROUND);
        }
        if (abs95CheckBox != null && !abs95CheckBox.isSelected()) {
            abs95CheckBox.setSelected(true);
            plot.getRenderer().addAnnotation(abs95Annotation, Layer.BACKGROUND);
        }
        if (abs99CheckBox != null && !abs99CheckBox.isSelected()) {
            abs99CheckBox.setSelected(true);
            plot.getRenderer().addAnnotation(abs99Annotation, Layer.BACKGROUND);
        }
    }
    ...
    public void hideABS() {
        if (abs95CheckBox != null && abs95CheckBox.isSelected()) {
            abs95CheckBox.setSelected(false);
            plot.getRenderer().removeAnnotation(abs95Annotation);
        }
        if (abs99CheckBox != null && abs99CheckBox.isSelected()) {
            abs99CheckBox.setSelected(false);
            plot.getRenderer().removeAnnotation(abs99Annotation);
        }
    }

    private JLabel createNonCheckBoxLabel(final XYShapeAnnotation annotation, String label, Stroke stroke, Color color) {
        JLabel l = new JLabel(label, new AnnotationIcon(stroke, color), JLabel.CENTER);
    ...
    public void addPoint(int series, double x, double y, PropertyMolecule molecule) {
        ...
        if(!defaultDomainRange.contains(x)){
            if(x>defaultDomainRange.getUpperBound()){
                defaultDomainRange = new Range(minAnnotationX,x);
            }else{
                defaultDomainRange = new Range(x,maxAnnotationX);
            }
            reset();
        }
        if(!defaultRangeRange.contains(y)){
            if(y>defaultRangeRange.getUpperBound()){
                defaultRangeRange = new Range(minAnnotationY,y);
            }else{
                defaultRangeRange = new Range(y,maxAnnotationY);
            }
            reset();
        }
    }
    ...
    public void resetRanges(){
        ...
        defaultDomainRange = new Range(minAnnotationX, maxAnnotationX);
        defaultRangeRange = new Range(minAnnotationY, maxAnnotationY);
    ...
    public static void main(String[] args) {
        final EganEgg plot = new EganEgg("<html><body><font color=\"#0000FF\">Absorption-CNS</font></body></html>", "2D tPSA", "XLogP", true, true, true);

        int series = plot.addSeries("BIO Number");
        File file = new File("/Users/jfeng1/Datasets/Arqule/My_instock_psa_xlogp.csv");
        try {
            ...
            while((line=lineNumberReader.readLine())!=null){
                ...
                oechem.OEParseSmiles(mol,"C1CCCC1");
                String[] arguments = line.split("\t");
                PropertyMolecule molecule = new PropertyMolecule(mol);
                molecule.setName(arguments[0]);
                float psa = Float.parseFloat(arguments[2]);
                float xlogp = Float.parseFloat(arguments[1]);
                if(psa<200) {
                    plot.addPoint(series, psa, xlogp, molecule);
                }
            }
        } catch (IOException e) {
            e.printStackTrace();
...
//            OEGraphMol mol = new OEGraphMol();
//            oechem.OEParseSmiles(mol,"CCCC");
//            plot.addPoint(series, abs *200, Math.random()*8, new PropertyMolecule(mol));
//        }

        final JFrame f = new JFrame("Egan Egg Example");
