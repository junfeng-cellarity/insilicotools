...
if __name__ == "__main__":
    ...
    if gethostname() == "compchem":
        default_directory = "/home/jfeng/datasets/BuildingBlocks/Pharmaron"
    else:
        default_directory = "/Users/junfeng/Downloads/"
    #sdf_filename = "Pharmaron-BBList-2020-08-21.sdf"
    ...
    database = os.path.join(default_directory,"Pharmaron_BB_current.sdf")
...
#        sys.exit(1)
    ...
    try:
        ...
        while OEReadMolecule(ifs,mol):
            ...
            catalogue_id = OEGetSDData(mol,"StockID").strip()
        ...
        conn = psycopg2.connect(database='Pharmaron',user='medchem',host='10.74.2.128',password='medchem')
        cursor = conn.cursor()
        cursor.execute("drop table if exists pharmaron_chemicals;")
        cursor.execute("drop schema if exists rdk cascade;")
        cursor.execute("create table if not exists pharmaron_chemicals (chemical_id serial primary key not null, molfile text, catalogue_id char(30),cas char(100), mdl char(30), mw real, psa real, clogp real);")
        ...
        while OEReadMolecule(ifs,mol):
            ...
            sys.stderr.flush()
            catalogue_id = OEGetSDData(mol,"ID_STOCKPURCHASE").strip() #use "id" for catalogue sdf
            cas = OEGetSDData(mol,"CAS").strip()
            mdl = OEGetSDData(mol,"MDL").strip()
            print(catalogue_id,cas,mdl)
            if len(catalogue_id)==0:
                continue
            if mol.NumAtoms() > 0:
                ...
                cursor.execute(sql.SQL("insert into pharmaron_chemicals (molfile,catalogue_id,cas, mdl, psa,mw,clogp) values (%s,%s,%s,%s, %s, %s,%s)"),[molfile,catalogue_id,cas, mdl,psa,mw,clogp])
        ...
        sys.stderr.write("\n")
        cursor.execute("select * into rdk.mols from (select chemical_id,mol_from_ctab(molfile::cstring) m from pharmaron_chemicals) tmp where m is not null;")
